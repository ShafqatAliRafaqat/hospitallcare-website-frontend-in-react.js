...
class LabTest extends Component{
    ...
    fetchCurrentLabTest= () => {
		...
		let { fetchCurrentLabTest, dispatch, errorHandler } = this.props;

        let customerId = this.props.user.customer.id;
        let token = this.props.user.access_token;

        fetchCurrentLabTest(customerId, token).then(res => {
			this.setState({
				diagnostics_data	: res.data.data,
			});
			dispatch({
				type: actions.FETCH_CURRENT_LAB_TEST,
        ...
        if(diagnostics_data.length ==0){
            return (
                <div class="strip_list wow fadeIn">
                    <p className="text-center">There is no Diagnostic Data</p>
                </div>
            );
        }
...
const mapDispatchToProps = dispatch => {
	return {
		dispatch: dispatch,
		fetchCurrentLabTest: (id, token)  => actions.fetchCurrentLabTest(id, token),

	};
};

export default connect( mapStateToProps, mapDispatchToProps )(LabTest);