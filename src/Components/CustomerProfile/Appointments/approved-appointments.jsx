...
class Appointments extends Component{
    ...
    fetchApprovedAppointment= () => {
		...
		let { fetchApprovedAppointment, dispatch, errorHandler } = this.props;

        let customerId  = this.props.user.customer.id;
        let token       = this.props.user.access_token;

        fetchApprovedAppointment(customerId, token).then(res => {
			this.setState({
...
const mapDispatchToProps = dispatch => {
	return {
		dispatch: dispatch,
		fetchApprovedAppointment: (id, token)     => actions.fetchApprovedAppointment(id, token),
