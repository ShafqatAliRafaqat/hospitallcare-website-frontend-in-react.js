...
class Vlog extends Component{
	...
	componentDidMount() {
      ...
      getAllVlogs = (search) => {
        ...
        let { getAllVlogs, dispatch, errorHandler, alertify } = this.props;
		getAllVlogs(search).then(res => {
            this.setState({
			...
			if(res.data.meta.total == 0){
				alertify.error("There is no Vlogs Right now");
			}
			window.scrollTo(0, 0);
            dispatch({
                type: actions.GET_VLOGS,
	...
	handlePageChange = (pageNumber) => {
		...
		this.getAllVlogs(search + "page=" + 	pageNumber , actions.GET_VLOGS);
	}
	renderVlogs = () =>{
			...
			return(
				...
				<div className="col-lg-4">
					<div className="box_list wow fadeIn">
						...
						<ul>
							<li><Link to={{pathname:`/vlog-detail/${slugify(m.title)}/${m.id}`}}><i className="icon_pencil-edit"></i>{m.updated_by}</Link></li>
							<li><Link to={{pathname:`/vlog-detail/${slugify(m.title)}/${m.id}`}}></Link></li>
...
const mapDispatchToProps = dispatch => {
    return {
        dispatch: dispatch,
        getAllVlogs: (search) => actions.getAllVlogs(search),
    };
};

export default connect(mapDispatchToProps)(Vlog);